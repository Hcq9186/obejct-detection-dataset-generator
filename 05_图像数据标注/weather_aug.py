...
annotation_path = config[config['dataset_like']]['annotation_save_path']
path = config[config['dataset_like']]['pic_save_path']


annotation_savedpath = './VOCdevkit_Weather/VOC2007/Annotations/'
savedpath = './VOCdevkit_Weather/VOC2007/JPEGImages/'
...
for root,dirs,files in os.walk(annotation_path):
    for file in files:
        if os.path.splitext(file)[1] == '.xml':
            annotation_list.append(os.path.join(file))

for count in range(times):
    images_aug = seq.augment_images(imglist)
    for index in range(len(images_aug)):
        # 保存图片 文件名和源文件相同
        filename = str(count) + str(filelist[index])
        annotation_filename = str(count) + str(annotation_list[index])
        cv2.imwrite(savedpath + filename, images_aug[index])
        for annotation_name in annotation_list:
            copyfile(annotation_path+annotation_name, annotation_savedpath + annotation_filename)
