...
chrome_driver_path = os.path.join(project_path, '00_算法参数配置/chromedriver_win32/chromedriver.exe')
print(chrome_driver_path)
base_url = config['base_url']

home_page = 'https://graph.baidu.com/pcpage/index?tpl_from=pc'
seed_img_dir = os.path.join(project_path, config['seed_img_dir'])
seed_save_dir = os.path.join(project_path, config['seed_save_dir'])


def prepare_seed_imgs():
    seed_imgs_url_list = []
    save_dir_list = []
    for file_name in os.listdir(seed_img_dir):
